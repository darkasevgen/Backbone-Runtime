...
class ViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func getModelNames(mlmodelPaths: [String]) -> ([URL], [String]) {
        var modelUrls: [URL] = []
        var modelNames: [String] = []

        for modelPath in mlmodelPaths {
            let url = URL(fileURLWithPath: modelPath)
            modelUrls.append(url)
            modelNames.append(processingName(name: url.modelName))
        }

        return (modelUrls, modelNames)
    }
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        let urls: [URL], names: [String]
        (urls, names) = getModelNames(mlmodelPaths: modelPaths)
        
        let countRuns: Int = 100
        
        var name: String, url: URL
        ...
        for i in 0...names.count - 1 {
            ...
            for _ in 0...countRuns - 1 {
                (_, _, timeInSec) = inferenceOneModel(modelURL: url, image: image)
                cumsumTime.append(Double(timeInSec))
            }
            
            (probs5k, indices5k, _) = inferenceOneModel(modelURL: url, image: image)
            ...
            let alert = UIAlertController(title: name, message: info, preferredStyle: .alert)
